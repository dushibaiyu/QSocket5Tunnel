...
AES::AES(const QByteArray & password, const QByteArray & salt, AES::KeySize keySize)
{
    ...
    key += QCryptographicHash::hash(salt,QCryptographicHash::Md5);
    key = QCryptographicHash::hash(key,QCryptographicHash::Sha3_256);
    RunKeyExpansion(keySize, (unsigned char *)key.data());
}
...
void AES::fileEncrypt(QFile *fin, QFile *fout)
{
...
void AES::fileDecrypt(QFile *fin, QFile *fout)
{
    unsigned char input[16];
    unsigned char output [16];
...
void AES::dataEncrypt(QByteArray &data)
{
    ...
    {
        memset((void *)input,0,16);
        size = buf.read((char *)input,16);
        Cipher(input,output);
        data.append((char *)output,16);
    }

}

void AES::dataDecrypt(QByteArray &data)
{
    if (data.isEmpty()) return;
    unsigned char input[16];
    unsigned char output [16];
...
void AES::RunKeyExpansion(KeySize keySize, unsigned char* keyBytes)
{
...
void AES::SetNbNkNr(KeySize keyS)
{
